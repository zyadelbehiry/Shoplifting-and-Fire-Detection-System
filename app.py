...
def predict_shoplifting(vid_path):
    ...
    global rnn_model
    rnn_model_script=keras.models.load_model('D:\GP using FastAPI-last\shopliftingmodel.h5', compile=False)
    global feature_model_script
    feature_model_script=keras.models.load_model('D:\GP using FastAPI-last\mobilnet.h5')
    ...
    try:
        while True:
            try:
                data = await websocket.receive_text()
                header, encoded = data.split(",", 1)
                img_arr = np.frombuffer(base64.b64decode(encoded), dtype=np.uint8)
                img = cv2.imdecode(img_arr, cv2.IMREAD_COLOR)
                results = model.predict(source=img, save=False, conf=0.4, show=False, stream=True)
                for r in results :
...
detect_dir = r"D:\GP using FastAPI-last\templates\videos\fire_detection"
predictno = len(os.listdir(detect_dir))

def get_image_path():
    if predictno == 1:
        return f"D:\GP using FastAPI-last\templates\videos\fire_detection"
    else:
        return f"D:\GP using FastAPI-last\templates\videos\fire_detection\predict{predictno}"

def detect_video(video_path):
    print("i entered the model")
    model_path = "best.pt"
    model = YOLO(model_path)
    model.predict(source=video_path, save=True, save_dir=get_image_path(), conf=0.4, show=False) 
    predictno += 1
